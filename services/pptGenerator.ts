...
export const generatePPT = async (user: UserProfile, plan: GeneratedPlan) => {
  ...
  const addHeader = (slide: PptxGenJS.Slide, title: string) => {
    slide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: '100%', h: 0.8, fill: { color: primaryColor } });
  ...
  const addFooter = (slide: PptxGenJS.Slide) => {
    ...
    // Note: addSlideNumber is deprecated in newer versions of pptxgenjs, removing to prevent build error.
    // Page numbers can be handled via defineSlideMaster if needed globally.
  };
  ...
  slide2.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.8, w: 12.3, h: 1.2, fill: { color: 'F3F3F3' } });
  ...
  swotBoxes.forEach(box => {
    slideSwot.addShape(pptx.ShapeType.rect, { x: box.x, y: box.y, w: 6.0, h: 2.4, fill: { color: box.color } });
  ...
  leftCol.forEach((item: string, i: number) => {
     slide3.addShape(pptx.ShapeType.rect, { x: 0.5, y: 2.3 + (i * 1.5), w: 5.8, h: 1.3, fill: { color: 'FAFAFA' }, line: {color: 'CCCCCC'} });
  ...
  rightCol.forEach((item: string, i: number) => {
    slide3.addShape(pptx.ShapeType.rect, { x: 6.8, y: 2.3 + (i * 1.5), w: 6.0, h: 1.3, fill: { color: 'FAFAFA' }, line: {color: 'CCCCCC'} });
  ...
  safeArray(plan.timeline?.content).forEach((t: string, i: number) => {
     slide4.addShape(pptx.ShapeType.rightArrow, { x: 0.5 + (i * 3.5), y: 2.2, w: 3, h: 0.8, fill: { color: secondaryColor } });
  ...
  slide5.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.8, w: 12.3, h: 1.5, fill: { color: 'F3E5F5' } });
  ...
  slide5.addShape(pptx.ShapeType.roundRect, { x: 9, y: 4.5, w: 3.5, h: 2, fill: { color: primaryColor } });
